/**
 * Carga los puntos de medida existentes en el tramo A1-A2 desde el archivo A1-A2.geojson
 * @param callback
 */
function loadPMJSON(callback) {
    var xobj = new XMLHttpRequest();
    xobj.overrideMimeType("application/json");
    xobj.open('GET', './locations/A1-A2.geojson', true);
...
/**
 * Carga las estaciones de contaminacion desde el archivo estaciones_contaminacion.geojson
 * @param callback
 */
function loadEstacionesJSON(callback) {
    var xobj = new XMLHttpRequest();
    xobj.overrideMimeType("application/json");
    xobj.open('GET', './locations/estaciones_contaminacion.geojson', true);
    xobj.onreadystatechange = function () {
        if (xobj.readyState == 4 && xobj.status == "200") {
            callback(xobj.responseText);
        }
...
/**
 * Cambia el escemario de la predicción
 */
function change_state() {
    ...
    getPrediction(datetime, (result) => {
        console.log(result);
        var txtSpeed = document.getElementById('txtSpeed');
        txtSpeed.value = result;
        drawWithInputValue()
    });
...
/**
 * Abre/expande el menu de creacion de escenario
 */
function openNav() {
...
/**
 * Cierra/oculta el menu de creacion de escenario
 */
function closeNav() {
...
/**
 * Crea variable json para representar los valores de NO2
 * @param nitro_array
 * @param estaciones_json
 * @returns {{features: [], type: string}}
 */
function createNitroGeojson(nitro_array, estaciones_json) {
    var nitro_geojson = {
        "type": "FeatureCollection",
        "features": []
    };
    for (var i = 0; i < nitro_array.length; i++) {
        var estaciones_list = estaciones_json.features;
        var coordinates = [];
        for (var j = 0; j < estaciones_list.length; j++) {
            if (estaciones_list[j].properties.code == nitro_array[i].estacion) {
                coordinates = estaciones_list[j].geometry.coordinates
            }
        }
        var feature = {
            'type': 'Feature',
            'properties': {
                'hour': nitro_array[i].datetime,
                'mag': nitro_array[i].valor,
                'estacion': nitro_array[i].estacion,
            },
            'geometry': {
                'type': 'Point',
        ...
        nitro_geojson.features.push(feature)
    }
    return nitro_geojson
}

/***** Indicador de intensidad de tráfico
 * Código adaptado de: https://codepen.io/lossan/pen/GmzPWY********/
...
function drawSpeedometerColourArc(options) {
    ...
    drawSpeedometerPart(options, 1.0, "#4cd137", startOfGreen);
    drawSpeedometerPart(options, 0.9, "#fbc531", endOfGreen);
    drawSpeedometerPart(options, 0.9, "#e84118", endOfOrange);
...
function convertSpeedToAngle(options) {
    ...
    * equivelant angle.
    */
    var iSpeed = (options.speed / 625),
        iSpeedAsAngle = ((iSpeed * 20) + 10) % 180;
    console.log('xDDDDDDDDDDDd' + iSpeedAsAngle);
...
function drawWithInputValue(vx) {
